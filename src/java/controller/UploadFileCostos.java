...
public class UploadFileCostos extends HttpServlet {
    ...
    public void subirFichero(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException, FileUploadException {
        ...
        ReadFileCostos.leerArchivoExcel(new FileInputStream(uploadedFile));
        ReadFileCostos excel = new ReadFileCostos();

        TablaCostos c = new TablaCostos();
        TablaCostosDao dao = new TablaCostosDao();

        if (dao.GuardarTablaCostos(c) > 0) {
